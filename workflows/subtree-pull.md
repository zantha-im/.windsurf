...
- If MULTIPLE subtree consumer projects exist: use `ask_user_question` tool with options:
  ...
  - Individual project options (up to 3 shown)
  
If "Apply to all" is selected:
- Loop through each consumer project and execute Steps 1-5 for each
- Report summary at end: "Updated N projects: [list]"

Set the working directory (cwd) for all subsequent git commands to the selected/current project root.
